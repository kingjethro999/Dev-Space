                        ...
                        {message.role === 'assistant' ? (
                          <div className="text-xs leading-relaxed max-w-none">
                            <ReactMarkdown
                              ...
                              components={{
                                h1: ({ node, ...props }) => <h1 className="text-base font-extrabold text-foreground mt-4 mb-2 flex items-center gap-2 border-b border-border pb-1" {...props} />,
                                h2: ({ node, ...props }) => <h2 className="text-sm font-bold text-foreground mt-3.5 mb-2" {...props} />,
                                h3: ({ node, ...props }) => <h3 className="text-xs font-semibold text-foreground/90 mt-3 mb-1" {...props} />,
                                ul: ({ node, ...props }) => <ul className="list-disc pl-6 my-2 space-y-1 text-foreground/80" {...props} />,
                                ol: ({ node, ...props }) => <ol className="list-decimal pl-6 my-2 space-y-1 text-foreground/80" {...props} />,
                                li: ({ node, ...props }) => <li className="text-xs leading-relaxed" {...props} />,
                                p: ({ node, ...props }) => <p className="text-xs leading-relaxed text-foreground/85 my-2" {...props} />,
                                blockquote: ({ node, ...props }) => (
                                  <blockquote className="border-l-4 border-primary bg-primary/5 px-4 py-2 my-3 rounded-r-xl text-foreground/80 italic" {...props} />
                                ),
                                table: ({ node, ...props }) => (
                                ...
                                thead: ({ node, ...props }) => <thead className="bg-muted text-foreground font-bold border-b border-border" {...props} />,
                                ...
                                td: ({ node, ...props }) => <td className="px-4 py-2 text-foreground/80" {...props} />,
                                code: ({ node, inline, className, children, ...props }: any) => {
                                  const match = /language-(\w+)/.exec(className || '');
                                  const isInline = !match && !children.includes('\n');
                                  if (isInline) {
                                    return (
                                      <code className="bg-muted text-primary px-1.5 py-0.5 rounded-md font-mono text-[10px] border border-border" {...props}>
                                        {children}
                                      </code>
                                    );
                                  }
                                  return (
                                    <div className="relative my-4 group rounded-xl overflow-hidden border border-border bg-zinc-950 dark:bg-zinc-950/70 shadow-md">
                                      {match && (
                                        <div className="flex items-center justify-between px-4 py-1.5 bg-zinc-900/90 border-b border-border text-[9px] font-mono font-semibold text-muted-foreground">
                                      ...
                                      <pre className="p-3 overflow-x-auto text-[11px] text-zinc-100 font-mono leading-relaxed bg-zinc-950">
